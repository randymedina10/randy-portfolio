/* oxlint-disable typescript/no-floating-promises -- UI events intentionally dispatch worker messages asynchronously. */
...
function send(message) {
  if (!ready && message.type !== 'init')
    return Promise.reject(new Error('Python está cargando'));
  const id = ++seq;
...
function render(s) {
  ...
  if (kind === 'tres-en-raya') {
    ...
    document.getElementById('score').textContent =
      'Puntos ' + s.score + ' · Líneas ' + s.lines + ' · Nivel ' + s.level;
    [...board.children].forEach((b, i) => {
      b.style.backgroundColor = colors[s.board[Math.floor(i / 10)][i % 10]];
    });
    board.setAttribute(
      'aria-label',
      'Tablero de Tetris. ' + s.lines + ' líneas, ' + s.score + ' puntos.',
    );
    const nextGrid = document.getElementById('next-grid');
    nextGrid.replaceChildren();
    for (let r = 0; r < 2; r++)
      for (let c = 0; c < 4; c++) {
        const cell = document.createElement('span');
        cell.style.backgroundColor = colors[s.next[r]?.[c] || 0];
    ...
    document
      ...
      .forEach((b) => (b.disabled = s.over));
    clearTimeout(timer);
    if (!s.over && !s.paused && !document.hidden)
      timer = setTimeout(
        () => act({ type: 'action', action: 'tick' }),
        Math.max(120, 700 - (s.level - 1) * 60),
      );
  }
}
...
function setup() {
  ...
  worker.onmessage = ({ data }) => {
    ...
    ready = true;
    render(data.state);
    p.resolve(data.state);
  };
  worker.onerror = () => {
    statusEl.textContent = 'No se pudo iniciar el juego.';
    retry.hidden = false;
    ready = false;
  };
...
if (kind === 'tres-en-raya') {
  ...
  document
    ...
    .forEach((b) =>
      b.addEventListener('click', () =>
        act({ type: 'action', action: b.dataset.action }),
      ),
    );
  document.addEventListener('keydown', (e) => {
    ...
    if (actions[e.key] && ready) {
      e.preventDefault();
      act({ type: 'action', action: actions[e.key] });
    }
  });
  document.addEventListener('visibilitychange', () => {
    if (document.hidden && ready && state && !state.paused && !state.over)
      act({ type: 'action', action: 'pause' });
  });
  window.addEventListener('blur', () => {
    if (ready && state && !state.paused && !state.over)
      act({ type: 'action', action: 'pause' });
  });
}
...
if (document.modelContext?.registerTool) {
  ...
  register({
    ...
    description: 'Lee el tablero y estado del juego visible.',
    inputSchema: {
      type: 'object',
      properties: {},
  ...
  register({
    ...
    execute: async (input) => {
      ...
      return send({ type: 'action', action: input.action });
    },
  });
